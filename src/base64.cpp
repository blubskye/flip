...
const bool Base64::Decode(const std::string &encoded, std::vector<unsigned char> &data)
{
	if(encoded.size()>0)
	{
		...
		rval=mbedtls_base64_decode(0,0,&datalength,(unsigned char *)encoded.c_str(),encoded.size());

		data.resize(datalength,0);

		rval=mbedtls_base64_decode(&data[0],data.size(),&datalength,(unsigned char *)encoded.c_str(),encoded.size());
...
const bool Base64::Encode(std::vector<unsigned char> &data, std::string &encoded)
{
	if(data.size()>0)
	{
		...
		rval=mbedtls_base64_encode(0,0,&encodedlength,&data[0],data.size());
		encodeddata.resize(encodedlength,0);

		rval=mbedtls_base64_encode(&encodeddata[0],encodeddata.size(),&encodedlength,&data[0],data.size());
