...
int main(int argc, char *argv[])
{
	...
	signal(SIGPIPE,SIG_IGN);	// H-SIGPIPE: prevent crash on abrupt client disconnect
#endif
	...
	chmod((global::basepath+"flip.db3").c_str(), 0600);
	// TODO: encrypt RSAPrivateKey with user passphrase (PBKDF2/Argon2 -> AES)
	// For now, at least log a warning if plaintext keys are detected
	// H9: Set SQLite busy timeout so concurrent DB access retries for 5 s
	// instead of returning SQLITE_BUSY immediately, avoiding spurious errors.
	global::db.SetBusyTimeout(5000);
	global::daemon=false;
	...
	global::log.OpenFile();
	global::log.Rotate(global::basepath+"flip","log");
