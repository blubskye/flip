...
IRCClientThread::IRCClientThread(const int sock, const int connectiontype, IRCClientConnection::ssl_client_info *ssl):m_socket(sock),m_contype(connectiontype),m_ssl(ssl),m_tempbuffer(4096,0)
{
	...
	{
		mbedtls_ssl_set_bio(&(ssl->m_ssl),&m_socket,mbedtls_net_send,0,mbedtls_net_recv_timeout);
	}
}
...
void IRCClientThread::thread()
{
	...
	if(m_contype==IRCClientConnection::CON_SSL)
	{
		...
		mbedtls_ssl_conf_ciphersuites(&(m_ssl->m_ssl_config),mbedtls_ssl_list_ciphersuites());
		mbedtls_ssl_setup(&(m_ssl->m_ssl),&(m_ssl->m_ssl_config));
		mbedtls_ssl_handshake(&(m_ssl->m_ssl));
		if(rval!=0)
		{
...
const bool IRCClientThread::Disconnect()
{
	...
	if(m_ssl)
	{
		mbedtls_ssl_free(&m_ssl->m_ssl);
		mbedtls_ssl_config_free(&m_ssl->m_ssl_config);
		mbedtls_ctr_drbg_free(&m_ssl->m_ctr_drbg);
		mbedtls_entropy_free(&m_ssl->m_entropy);
		delete m_ssl;
		m_ssl=0;
	}
	m_wantdisconnect=false;
...
const int IRCClientThread::SocketReceive()
{
	...
	if(IsConnected() && m_contype==IRCClientConnection::CON_UNSECURE)
	{
		...
		if(rval>0)
		{
			if(m_receivebuffer.size()+(size_t)rval>8192) // 8KB max line
			{
				Disconnect();
				return -1;
			}
	...
	{
		int rval=mbedtls_ssl_read(&(m_ssl->m_ssl),(unsigned char *)&m_tempbuffer[0],m_tempbuffer.size());
		if(rval>0)
		{
			m_receivebuffer.insert(m_receivebuffer.end(),m_tempbuffer.begin(),m_tempbuffer.begin()+rval);
			//while(HandleReceivedData())
...
const int IRCClientThread::SocketSend()
{
	...
	{
		int rval=mbedtls_ssl_write(&(m_ssl->m_ssl),(unsigned char *)&m_sendbuffer[0],m_sendbuffer.size());
		if(rval>0)
		{
			m_sendbuffer.erase(m_sendbuffer.begin(),m_sendbuffer.begin()+rval);
		}
