...
const bool IRCChannel::ValidName(const std::string &name)
{
	...
	// M2: cap channel name length at 200 chars to prevent resource exhaustion.
	return (name.find_first_of(invalidchars)==std::string::npos && name.size()>0 && name.size()<=200);
}
