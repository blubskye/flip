...
#include <ctime>	// C8: time() for WaitForBytes timeout
#include <sstream>
#include <algorithm>
#include <cstdarg>
#include <cstring>
...
void Connection::DoReceive()
{
	if(IsConnected())
	{
		// C7: cap receive buffer at 10 MB to prevent memory exhaustion from a
		// misbehaving or malicious Freenet node sending an unbounded data stream.
		static const std::vector<char>::size_type MAX_RECV_BUFFER = 10 * 1024 * 1024;
		...
		if(m_isssl==false)
		{
			...
			if(len>0)
			{
				if(m_receivebuffer.size()+len<=MAX_RECV_BUFFER)
				{
					m_receivebuffer.insert(m_receivebuffer.end(),m_tempbuffer.begin(),m_tempbuffer.begin()+len);
				}
				// If the buffer is full we drop the data and disconnect; the FCP
				// protocol is line/message-framed so a partial message is unrecoverable.
				else
				{
					Disconnect();
				}
		...
		else
		{
			...
			if(len>0)
			{
				if(m_receivebuffer.size()+(size_t)len<=MAX_RECV_BUFFER)
				{
					m_receivebuffer.insert(m_receivebuffer.end(),m_tempbuffer.begin(),m_tempbuffer.begin()+len);
				}
				else
...
const bool Connection::Receive(Message &message)
{
	...
	if(MessageReady(endpos,endlen)==true)
	{
		...
		if(fields.size()>1)
		{
			// H8: cap field count to prevent OOM from a malicious FCP message
			// with thousands of key=value lines.  256 fields is well above any
			// legitimate FCP message size.
			std::vector<std::string>::size_type fieldcount=0;
			static const std::vector<std::string>::size_type MAX_FIELDS=256;
...
{
	// C8: impose a 30-second absolute timeout on WaitForBytes so a slow or
	// silent Freenet node cannot stall the caller indefinitely.  Uses POSIX
	// time() which is always available; <ctime> is included via standard headers.
	const time_t deadline=time(nullptr)+30;
