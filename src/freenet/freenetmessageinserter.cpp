...
FreenetMessageInserter::FreenetMessageInserter(FreenetConnection *connection, FCPv2::Connection *fcp):IFCPConnected(fcp,connection),IPeriodicProcessor(connection),IFCPMessageHandler(connection,"MessageInserter")
{
	...
	FLIPEventSource::RegisterFLIPEventHandler(FLIPEvent::EVENT_IRC_CHANNELNOTICE,this);
	FLIPEventSource::RegisterFLIPEventHandler(FLIPEvent::EVENT_IRC_PRIVATEMESSAGE,this);
	FLIPEventSource::RegisterFLIPEventHandler(FLIPEvent::EVENT_IRC_PRIVATENOTICE,this);
	...
	FLIPEventSource::RegisterFLIPEventHandler(FLIPEvent::EVENT_IRC_NICKCHANGE,this);
...
{
	if((flipevent.GetType()==FLIPEvent::EVENT_IRC_CHANNELMESSAGE || flipevent.GetType()==FLIPEvent::EVENT_IRC_CHANNELNOTICE) && m_fcp->IsConnected())
	{
		std::map<std::string,std::string> params=flipevent.GetParameters();
		int id=0;
		StringFunctions::Convert(params["localidentityid"],id);

		StartChannelInsert(id,params["channel"],params["message"],flipevent.GetType());
	...
	else if((flipevent.GetType()==FLIPEvent::EVENT_IRC_PRIVATEMESSAGE || flipevent.GetType()==FLIPEvent::EVENT_IRC_PRIVATENOTICE) && m_fcp->IsConnected())
	{
		std::map<std::string,std::string> params=flipevent.GetParameters();
		...
		// H1-compat: the ircserver sets "legacycompat"="true" when the user ran
		// /1024 <nick> for this recipient, requesting PKCS#1 v1.5 padding so that
		// pre-H1 FLIP peers (1024-bit keys, old encryption) can still decrypt.
		bool legacycompat=(params.count("legacycompat") && params.at("legacycompat")=="true");
		StartPrivateInsert(localidentityid,recipientidentityid,params["message"],flipevent.GetType(),legacycompat);
	...
	else if(flipevent.GetType()==FLIPEvent::EVENT_IRC_KEEPALIVE && m_fcp->IsConnected())
	{
		...
		StartKeepAliveInsert(localidentityid,params["nick"],params["channels"]);

		return true;
	}
	else if(flipevent.GetType()==FLIPEvent::EVENT_IRC_NICKCHANGE && m_fcp->IsConnected())
	{
		std::map<std::string,std::string> params=flipevent.GetParameters();
		int localidentityid=0;
		...
		StartNickChangeInsert(localidentityid,params["nick"]);
...
void FreenetMessageInserter::StartChannelInsert(const int localidentityid, const std::string &channel, const std::string &message, const int &eventtype)
{
	...
	if(m_identitykeys[localidentityid]!="")
	{
		...
		switch(eventtype)
		{
		case FLIPEvent::EVENT_IRC_CHANNELNOTICE:
			fm["type"]="channelnotice";
			break;
		case FLIPEvent::EVENT_IRC_CHANNELMESSAGE:
		default:
...
void FreenetMessageInserter::StartPrivateInsert(const int localidentityid, const int recipientidentityid, const std::string &message, const int &eventtype, const bool legacycompat)
{
	RSAKeyPair rsa;
	std::string idstr("");
	...
	// H1-compat: use legacy PKCS#1 v1.5 context when legacycompat=true so that
	// old FLIP peers (running without the H1 patch) can decrypt the message.
	if(publickey!="" && rsapublickey!="" && rsa.SetFromEncodedPublicKey(rsapublickey,legacycompat))
	{
		if(m_identitykeys[localidentityid]!="")
		{
			std::string indexstr("0");
			...
			switch(eventtype)
			{
			case FLIPEvent::EVENT_IRC_PRIVATENOTICE:
				fm["type"]="privatenotice";
				break;
			case FLIPEvent::EVENT_IRC_PRIVATEMESSAGE:
			default:
...
void FreenetMessageInserter::StartKeepAliveInsert(const int localidentityid, const std::string &nick, std::string &channels)
{
	SQLite3DB::Statement st;
	FCPv2::Message mess("ClientPut");
	LoadLocalIdentityPrivateKey(localidentityid);
...
void FreenetMessageInserter::StartNickChangeInsert(const int localidentityid, const std::string &nick)
{
	SQLite3DB::Statement st;
	FCPv2::Message mess("ClientPut");
	LoadLocalIdentityPrivateKey(localidentityid);
	...
	if(m_identitykeys[localidentityid]!="")
	{
		...
		m_log->Debug("FreenetMessageInserter::StartInsert started insert of nickchange "+mess["Identifier"]);	
	}
}
