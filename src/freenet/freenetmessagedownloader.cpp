			...
			if(m_fcp->WaitForBytes(100,datalength))
			{
				...
				if(m_fcp->Receive(data,datalength))
				{
					...
					if(FreenetMessage::TryParse(std::string(data.begin(),data.end()),fm))
					{
						...
						if(fm["type"]=="channelmessage" || fm["type"]=="channelnotice")
						{
							...
							else if(fm["type"]=="channelnotice")
							{
								DispatchFLIPEvent(FLIPEvent(FLIPEvent::EVENT_FREENET_NEWCHANNELNOTICE,params));
							}
						}
						else if(fm["type"]=="privatemessage" || fm["type"]=="privatenotice")
						{
							params["identityid"]=idparts[1];
							...
							if(fm["type"]=="privatemessage")
							{
								DispatchFLIPEvent(FLIPEvent(FLIPEvent::EVENT_FREENET_NEWPRIVATEMESSAGE,params));
							}
							else if(fm["type"]=="privatenotice")
							{
								DispatchFLIPEvent(FLIPEvent(FLIPEvent::EVENT_FREENET_NEWPRIVATENOTICE,params));
							}
						}
						...
						else if(fm["type"]=="keepalive")
						{
							...
							if(fm.Headers().find("name")!=fm.Headers().end())
							{
								params.clear();
								params["name"]=fm["name"];
								params["sentdate"]=fm["sentdate"];
								params["identityid"]=idparts[1];
								DispatchFLIPEvent(FLIPEvent(FLIPEvent::EVENT_FREENET_IDENTITYNAME,params));
							}
						}
...
void FreenetMessageDownloader::StartRequest(const int identityid, const std::string &publickey, const std::string &date, const int edition)
{
	if(publickey!="")
	{
		...
		if(publickey.size()<4)
		{
			m_log->Error("FreenetMessageDownloader::DownloadMessage Invalid publickey length");
			return;
		}
