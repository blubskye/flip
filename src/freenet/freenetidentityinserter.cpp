...
FreenetIdentityInserter::FreenetIdentityInserter(FreenetConnection *connection, FCPv2::Connection *fcp):IFCPConnected(fcp,connection),IPeriodicProcessor(connection),IFCPMessageHandler(connection,"IdentityInserter"),m_inserting(false),m_lastactivity()
{
	...
	option.GetInt("MessageInsertPriority",m_insertpriority);
...
const bool FreenetIdentityInserter::HandleFCPMessage(FCPv2::Message &message)
{
	if(message["Identifier"].find(m_fcpuniqueidentifier)==0)
	{
		...
		if(message.GetName()=="PutSuccessful")
		{
			...
			SQLite3DB::Statement st=m_db->Prepare("INSERT INTO tblLocalIdentityInsert(LocalIdentityID,Day,InsertIndex) VALUES(?,?,?)");
			st.Bind(0,id);
			st.Bind(1,idparts[2]);
			st.Bind(2,idparts[3]);
			st.Step();
		...
		else if(message.GetName()=="PutFailed")
		{
			...
			if(message["Code"]=="9")	// collided with existing data, mark index inserted
			{
				SQLite3DB::Statement st=m_db->Prepare("INSERT INTO tblLocalIdentityInsert(LocalIdentityID,Day,InsertIndex) VALUES(?,?,?)");
				st.Bind(0,id);
				st.Bind(1,idparts[2]);
...
void FreenetIdentityInserter::RemoveRequest(const std::string &identifier)
{
	FCPv2::Message mess("RemovePersistentRequest");
	mess["Identifier"]=identifier;
	m_fcp->Send(mess);
}
...
void FreenetIdentityInserter::StartIDInsert(const int id)
{
	...
	if(privatekey!="")
	{
		...
		std::string insertprioritystr("2");
		std::string identityindexstr("0");
		...
		st=m_db->Prepare("SELECT RSAPublicKey, Name FROM tblLocalIdentity WHERE LocalIdentityID=?;");
		st.Bind(0,id);
		st.Step();
		if(st.RowReturned())
		{
			st.ResultText(0,rsapublickey);
			st.ResultText(1,m_activeids[id].m_name);
		}

		st=m_db->Prepare("SELECT IFNULL(MAX(InsertIndex)+1,0) FROM tblLocalIdentityInsert WHERE LocalIdentityID=? AND Day=?;");
		st.Bind(0,id);
		st.Bind(1,now.Format("%Y-%m-%d"));
		st.Step();
		if(st.RowReturned())
		{
			st.ResultText(0,identityindexstr);
		}
		...
		StringFunctions::Convert(m_insertpriority,insertprioritystr);

		FCPv2::Message mess("ClientPut");
		mess["URI"]="SSK@"+privatekey.substr(4)+m_messagebase+"|"+now.Format("%Y-%m-%d")+"|Identity-"+identityindexstr;
		mess["Identifier"]=m_fcpuniqueidentifier+"|"+idstr+"|"+now.Format("%Y-%m-%d")+"|"+identityindexstr+"|"+mess["URI"];
		mess["UploadFrom"]="direct";
		mess["IgnoreUSKDatehints"]="true";
		mess["DataLength"]=datalengthstr;
		mess["Metadata.ContentType"]="";
		mess["PriorityClass"]=insertprioritystr;
