					...
					if(IRCNick::IsValid(fm["name"]))
					{
						...
						if(fm["rsapublickey"]!="" && rsa.SetFromEncodedPublicKey(fm["rsapublickey"])==true)
						{
							...
							if(st.Step()==true)
							{
								...
								// dispatch event for identity name (may be same or changed)
								eventparams.clear();
								eventparams["name"]=fm["name"];
								eventparams["sentdate"]=idparts[2];
								eventparams["identityid"]=idparts[1];
								DispatchFLIPEvent(FLIPEvent(FLIPEvent::EVENT_FREENET_IDENTITYNAME,eventparams));
