...
void SetupDB(SQLite3DB::DB *db)
{
	...
	{
		db->Execute("INSERT INTO tblDBVersion(Major,Minor) VALUES(0,5);");
	}

	db->Execute("UPDATE tblDBVersion SET Major=0, Minor=5;");
	...
	db->Execute("CREATE TABLE IF NOT EXISTS tblLocalIdentityInsert(\
	...
	db->Execute("CREATE TABLE IF NOT EXISTS tblIdentity(\
				...
				Validated				BOOL CHECK(Validated IN(0,1)) DEFAULT 0\
				);");
